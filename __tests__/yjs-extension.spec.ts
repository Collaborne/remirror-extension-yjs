import { yUndoPluginKey } from '@collaborne/y-prosemirror';

import { extensionValidityTest, renderEditor } from 'jest-remirror';
import { WebrtcProvider } from 'y-webrtc';
import { Doc } from 'yjs';

// NB: Import the src/ directory, not any stale content from dist/
import { defaultDestroyProvider, YjsExtension, YjsOptions } from '../src';
...
describe('configuration', () => {
  ...
  it.each([
    ...
    { trackedOrigins: [] },
    { disableUndo: true }
  ])('throws when updating undo-related options (%o)', (option: any) => {
    const { manager } = create();
    const extension = manager.getExtension(YjsExtension);

    expect(() =>
      extension.setOptions({ ...option }),
